...
def view(game):
    ...
    play_button = Button(vw(50), vh(78), "Settings", h = vh(20), font = config.font_primary.xl)
    #TODO - make settings screen view (include theme and speed option) | Connect view with this button
    ...
    def draw():
        ...
        logo = utils.scale_image_maintain_ratio(title, h = title_h * (1 + title_scale))
        logo, rect = utils.rotate_by_center(logo, title_angle, vw(50), vh(22.5))
        game.gameDisplay.blit(logo, rect)

    def update():
        step["ticks"] += 1

        if step["ticks"] > 10e2: #Cap ticks to avoid big numbers for calculation
            step["ticks"] = 0
